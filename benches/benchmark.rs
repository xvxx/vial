...
pub fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("simple_get", |b| {
        b.iter(|| parse_fixture("tests/http/simple_GET.txt"))
    });
    c.bench_function("another_get", |b| {
        b.iter(|| parse_fixture("tests/http/another_GET.txt"))
    });
    c.bench_function("big_get", |b| {
        b.iter(|| parse_fixture("tests/http/big_GET.txt"))
    });
    c.bench_function("stacked_headers_get", |b| {
        b.iter(|| parse_fixture("tests/http/stacked_headers_GET.txt"))
    });
    c.bench_function("simple_post", |b| {
        b.iter(|| parse_fixture("tests/http/simple_POST.txt"))
    });
    c.bench_function("simple_post2", |b| {
        b.iter(|| parse_fixture("tests/http/simple_POST2.txt"))
    });
    ...
    c.bench_function("basic_cookies", |b| {
        b.iter(|| parse_fixture("tests/http/cookies_basic.txt"))
    });
    c.bench_function("cookies_only_once", |b| {
        b.iter(|| parse_fixture("tests/http/cookies_assign_only_once.txt"))
    });
    c.bench_function("cookies_escape", |b| {
        b.iter(|| parse_fixture("tests/http/cookies_escaping.txt"))
    });
    c.bench_function("cookies_ignore_escaping_error_and_return_orig_value", |b| {
        b.iter(|| {
            parse_fixture("tests/http/cookies_ignore_escaping_error_and_return_orig_value.txt")
        })
    });
    c.bench_function("cookies_ignore_non_values", |b| {
        b.iter(|| parse_fixture("tests/http/cookies_ignore_non_values.txt"))
    });
    c.bench_function("cookies_unencoded", |b| {
        b.iter(|| parse_fixture("tests/http/cookies_unencoded.txt"))
    });
    c.bench_function("json_post", |b| {
        b.iter(|| parse_fixture("tests/http/json_POST.txt"))
    });
}
